...
- **提成分批结算 + 可改金额 + 收款金额可改**(本轮追加,业主提"分成跟着定金/尾款分批给、实际金额常和计划不符、特例8000不值得新开版本",已上线 + push 到 `1c641d0`,migration **0021**):
  - 数据:`external_commission_entries.amount_settled`(分批已结累计)+ `commission_entries.amount_override`(业务员每单改金额)。
  - API:外部 settle 支持**部分金额**(本次实际,默认结清剩余)、可多次、累计 amount_settled、满额转 settled、每次独立出账+凭证;PATCH 改应付;**物化保留**已部分结算/已结算行(只删 pending 且 amount_settled=0,重建跳过已保留 source_line)。业务员 PATCH amount_override,工资条/mine 用 `coalesce(override,amount)`,重算保留 override。收款 charge 改金额复用已有 PATCH。
  - 前端:外部台账 应付/已结/未结分列 + 部分状态 + 改应付 + 结算填本次金额(多次);对账单分列;销售提成行内改金额;**ChargeSchedulePanel 每笔应收金额可改**(特例不新开版本)。
  - **端到端实测**:介绍人提成应付1000→改800(特例)→结300(未结500/pending)→结500(已结800/settled)→对账单800/800/0→两笔出账300+500;重算 billing 后已结算行不丢不重复。测试数据已清。
  - **业主用法**:特例就在那一单上改收款金额、改提成应付;分成定金给一部分(填本次实际)、尾款再结剩下;业务员提成也能按单改金额(仍进工资条)。
- **提成里程碑(共用收款阶段)**(本轮追加,业主提"收款有里程碑、每个提成也要有里程碑、可能多个人提成",2026-06-28,已上线+push 到 `62726cf`,migration **0022**):
  - spec:`docs/superpowers/specs/2026-06-28-commission-milestones-design.md`;业主选**共用收款阶段**(阶段在收款里程碑定一次,每条提成填各阶段%)。
  - 数据:`scheme_lines.milestone_split jsonb`(每条提成各阶段%)+ 提成 entries 加 `milestone_seq`。
  - 引擎:`@bh/shared splitCommissionByMilestones` 纯函数(空→整额/显式split含0/默认按收入占比/末笔吃余额)+ 6 单测(共 17 绿);外部+业务员物化 one_time 提成按阶段拆成每阶段一笔(milestone_seq+note带阶段名);**保留键带 milestone_seq**(阶段级保留已结算/部分结算,重算不丢不重复)。
  - 前端:规则行分成行加「里程碑分配」Modal(列出收款阶段填%,空=跟随收款比例);台账/对账单/销售提成显示阶段(介绍人提成·首付)。
  - **端到端实测**:EP 首付/尾款;介绍人提成 split 首付30/尾款70 → 成交1万 → 自动拆 介绍人·首付300 + 介绍人·尾款700 → 结首付300后重算 → 首付仍 settled、尾款 pending、正好2行不丢不重复;对账单按阶段显示。测试数据已清。
  - **业主用法**:在分成行点「里程碑分配」设各阶段拿多少%(如介绍人首付30尾款70、业务员只尾款100);成交后每条提成自动按阶段分笔,沿用分批结算/对账单/改金额。
...
- **✅ 待办「每单自定价格」已做完并上线**(commit `34809e9`):引擎 `fixed` 基准支持 `input_key`(有则读每单 inputs,否则用 rate 兜底);一次性卖断预设收入行 `input_key='price'`;`billing.ts` 把 `total_price_sgd>0` 注入 `inputs.price`(含 refresh 路径);seed 回填 5 业务一次性收入行 `input_key='price'`;12 单测绿。验证:EP 8000 单 → 首付 2400/尾款 5600 按真实总价拆、绑 step1/8。**每单填总价即可,首付/尾款/提成都按真实价。**
- **✅ 已上线 + 已 push**:本轮(收款名目 + 每单价格)代码 rebuild + `systemctl --user restart bh-prod` 上线 `bh.youjia.sg`;migration 到 **0018** 已在生产共用库;全部 commit `push origin master`(到 `34809e9`),**0 待推**。线上验证:7 个收款名目在、collection-items 路由正常。
- **DEMO/测试数据**:本轮测试数据已清干净(clients/cases/billing/charges = 0)。真实框架保留:2 公司、5 业务+方案版本(一次性收入已 input_key=price,首付/尾款里程碑绑名目)、7 收款名目、9 支出类别、5 分成对象。**业主可直接录真实数据。**
- **下次可做**:各业务真实价格/提成/成本配置(业主填)、新加坡报表精确模板(会计给)、CPF 费率(工资条扣项);非案件业务(按摩椅/床垫/加盟)按真实规则配方案版本。
